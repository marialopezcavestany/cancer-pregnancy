...
% This script runs the sensitivity analysis based on the protein
% concentrations and kinetic parameters optimized in the Monte Carlo
% simulation. It varies each of the fitted parameters from a quarter of the
% value to quadruple their value to asses to what extent the model output
% is affected by it.
...
LagTime = 0.1; %The lag time is 6 mins for model start up
...
protein.RJ = 34.82590622; 
protein.S3c = 120;
protein.SHP2 = 78.23736788;
protein.PPX = 66.0051475;
...
kin.k1 = 3.08E-03;               kin.k2f = 5.60E-05; 
kin.k2r = 5.60E-03;              kin.k3f = 0.063415984; 
kin.k3r = 0.2;                   kin.k4 = 5.00E-03;
kin.k5f = 0.0108715;             kin.k5r = 8.00E-01;
kin.k6 = 4.00E-01;               kin.kdeg = 2.57E-04; 
kin.deg_ratio = 13.07281945;     kin.k8f = 0.074430614; 
kin.k8r = 1.00E-01;              kin.k9f = 0.000879762; 
kin.k9r = 2.00E-01;              kin.k10 = 3.00E-03;
kin.k11f = 0.002400238;          kin.k11r = 2.00E-01;
kin.k12 =  3.00E-03;             kin.k13f = 2.00E-07; 
...
kin.k23 = 0.000665214;           kin.k24 = 0.00124969;
kin.k25_1 = 0.014087273;         kin.k25_2 = 4.00E+02;
kin.k26 = 1.00E-03;              kin.k27 = 6.12582E-05;
kin.k28 = 0.010887257;           kin.k29 = 1.00E-02;
kin.k30_1 = 0.001614721;         kin.k30_2 = 4.00E+02;
kin.k31 = 1.00E-03;              kin.k32 = 5.00E-04;
kin.k33 = 1.00E-02;              kin.k34 = 1E-04;
kin.Vratio = 5.00E-01;
...
%Set up the master matrix for the sensitivity analysis
Master = [];

%Set up random variation in protein expression and binding kinetics
random.randRJ = abs(protein.RJ - (protein.RJ*(randn(1,n)/3)));
random.randPPX = abs(protein.PPX - (protein.PPX*(randn(1,n)/3)));
random.randS3c = abs(protein.S3c - (protein.S3c*(randn(1,n)/3)));
random.randS3n = abs(protein.S3n - (protein.S3n*(randn(1,n)/3)));
random.randJAG = abs(protein.JAG - (protein.JAG*randn(1,n)));
random.randk2f = abs(kin.k2f - (kin.k2f*(randn(1,n)/3)));
random.randk2r = abs(kin.k2r - (kin.k2r*(randn(1,n)/3)));
random.randk4 = abs(kin.k4 - (kin.k4*(randn(1,n)/3)));
random.randk5f = abs(kin.k5f - (kin.k5f*(randn(1,n)/3)));
random.randk6 = abs(kin.k6 - (kin.k6*(randn(1,n)/3)));
random.randk8f = abs(kin.k8f - (kin.k8f*(randn(1,n)/3)));
random.randk11f = abs(kin.k11f - (kin.k11f*(randn(1,n)/3)));
random.randk12 = abs(kin.k12 - (kin.k12*(randn(1,n)/3)));
random.randk23 = abs(kin.k23 - (kin.k23*(randn(1,n)/3)));
random.randk25_1 = abs(kin.k25_1 - (kin.k25_1*(randn(1,n)/3)));
random.randk28 = abs(kin.k28 - (kin.k28*(randn(1,n)/3)));
random.randk30_1 = abs(kin.k30_1 - (kin.k30_1*(randn(1,n)/3)));
random.randk31 = abs(kin.k31 - (kin.k31*(randn(1,n)/3)));
random.randk33 = abs(kin.k33 - (kin.k33*(randn(1,n)/3)));
random.randk34 = abs(kin.k34 - (kin.k34*(randn(1,n)/3)));
random.randdeg_ratio = abs(kin.deg_ratio - (kin.deg_ratio*(randn(1,n)/3)));
...
JAG_RMSE = zeros(1, n);
%pSTAT_RSquared = zeros(1, n);
%JAG_RSquared = zeros(1, n);

plots.keepingtrack = [];
kinetics_stored = zeros(49,n);

plots.pSTAT_InVitro = [1 1.47 1.14 0.99 1.06];
plots.pSTAT_InVitro_t = [0 5 15 30 60];
plots.JAG_InVitro = [1 1.93 2.01 1.8];
plots.JAG_InVitro_t = [0 12 24 48];
...
for eye=1:n
    ...
    calculate.pSTAT_Model = [y(1,11) y(6,11) y(16,11) y(31,11) y(61,11)]; %This identifies the model outputs for pSTAT3 to compare to our in vitro results
    calculate.JAG_Model = [y(1,34) y(721,34) y(1441,34) y(2881,34)]; %This identifies the model outputs for total JAG1 (intracellular and extracellular) to compare to our in vitro results
    ...
    %Find_RSquared_pSTAT = fitlm(calculate.pSTAT_Model,plots.pSTAT_InVitro); %R Squared calculation for pSTAT3
    %pSTAT_RSquared(1,eye) = Find_RSquared_pSTAT.Rsquared.Ordinary; %Stores the pSTAT3 R Squared values for each iteration in a separate table
    %Find_RSquared_JAG = fitlm(calculate.JAG_Model,plots.JAG_InVitro); %R Squared calculation for JAG1
    %JAG_RSquared(1,eye) = Find_RSquared_JAG.Rsquared.Ordinary; %Stores the JAG1 R Squared values for each iteration in a separate table
